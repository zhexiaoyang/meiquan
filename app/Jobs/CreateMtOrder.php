...
class CreateMtOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $lock = Cache::lock("send_order_job:{$this->order->id}", 6);
        ...
        if ($order->fail_uu) {
            ...
            if (isset($check_uu['return_code']) && ($check_uu['return_code'] === 'ok') && ($money_uu >= 1) && ($addfee <= 10) ) {
                ...
                $order->total_money = $money_uu_total;
                $order->need_paymoney = $money_uu_need;
        ...
        if ($order->fail_dd) {
            ...
            if (isset($check_dd['code']) && ($check_dd['code'] === 0) && ($money_dd > 1) ) {
                ...
                $order->dada_order_id = $check_dd['result']['deliveryNo'];
        ...
        if ($order->fail_ss) {
            ...
            $order->fail_mt = "门店不支持闪送跑腿";
            $this->log("门店不支持「闪送」跑腿，停止「闪送」派单");
        } elseif (!$ss_switch) {
            $order->fail_mt = "门店关闭闪送跑腿";
