        ...
        if (!empty($order_request) && isset($order_request['body']['data']) && !empty($order_request['body']['data'])) {
            ...
            DB::transaction(function () use ($shop, $order_id, $order) {
                ...
                $order_wm_data = [
                    ...
                    "shipper_phone" => $data['order']['delivery_phone'] ?? "",
                    "status" => 4,
                    "estimate_arrival_time" => $data['estimate_arrival_time'] ?? 0,
                    "ctime" => $data['order']['create_time'],
                    "utime" => $data['order']['create_time'],
                    "delivery_time" => $data['order']['latest_send_time'],
                    "pick_type" => $pick_type,
                    "day_seq" => $data['order']['order_index'] ?? 0,
                    "invoice_title" => $data['order']['invoice_title'] ?? '',
                    "taxpayer_id" => $data['order']['taxer_id'] ?? '',
