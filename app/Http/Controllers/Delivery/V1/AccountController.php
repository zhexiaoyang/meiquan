...
class AccountController extends Controller
{
     ...
     * @data 2023/11/17 2:54 下午
     */
    public function operate_balance(Request $request)
    {
        // 1 充值记录，2 消费记录
        $money_type = (int) $request->get('money_type', 0);
        if (!in_array($money_type, [1, 2])) {
        ...
        $data = UserOperateBalance::select('id', 'description', 'money as amount', 'created_at')->where($where)->orderByDesc('id')->paginate($page_size);
