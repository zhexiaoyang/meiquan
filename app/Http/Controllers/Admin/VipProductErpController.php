...
class VipProductErpController extends Controller
{
    ...
    public function erp_cost(VipProduct $vipProduct)
    {
        ...
        switch ($setting->type) {
            ...
            case 2:
                return $this->jishitang($vipProduct, $setting);
        }
    }
    ...
    public function erp_cost_all(Request $request)
    {
        ...
        switch ($setting->type) {
            ...
            case 2:
                return $this->jishitang_all($setting);
        }
    }
    ...
    public function jishitang_all(ShopErpSetting $setting)
    {
        VipProduct::query()->where('shop_id', $setting->shop_id)->chunk(200, function ($products) use ($setting) {
            $product_ids = $products->pluck('upc')->toArray();
            $product_in = implode("','", $product_ids);
            \Log::info('同步ERP成本价条码1', [$product_in]);
            $data = DB::connection('jishitang')
                ->select("SELECT product_id as id,cost,upc FROM [dbo].[v_store_m_mtxs] WHERE [shop_id] = N'{$setting->sync_erp_shop_id}' AND [upc] IN ('{$product_in}')");
            $upcs = [];
            \Log::info('同步ERP成本价条码2', [$data]);
            if (!empty($data)) {
                foreach ($data as $v) {
    ...
    public function jishitang(VipProduct $product, ShopErpSetting $setting)
    {
        $exception_status = false;
        $exception_data = [
            'product_id' => $product->id,
        ...
        $data = DB::connection('jishitang')
            ->select("SELECT product_id as id,cost FROM [dbo].[v_store_m_mtxs] WHERE [门店ID] = N'{$setting->sync_erp_shop_id}' AND [upc] = N'{$product->upc}'");
        \Log::info('单个药品同步ERP成本价', $data);
