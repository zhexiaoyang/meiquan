...
class TakeoutImMessageTask extends Task
{
    ...
    //
    public function __construct($message_id, $user_id, $other = true)
    {
    ...
    {
        \Log::info("TakeoutImMessageTask-开始执行");
        ...
        if ($fd_str = Redis::hget($this->redis_key_user, $this->user_id)) {
            ...
            if (empty($user_fds)) {
                ...
                \Log::info("TakeoutImMessageTask|user_id没有链接，删除掉|user_id:{$this->user_id}");
            } else {
                if (count($user_fds) < count($fds)) {
                    $new_fd_str = implode(',', $user_fds);
                    Redis::hset($this->redis_key_user, $this->user_id, $new_fd_str);
                    \Log::info("TakeoutImMessageTask|user_id的fd有减少，重新赋值|原fd:{$fd_str}，新fd:{$new_fd_str}");
                }
            }
        }
        // 往其它服务器推送
        \Log::info("TakeoutImMessageTask|往其它服务器推送|user_id:{$this->user_id}|other:{$this->other}");
        if ($this->other) {
