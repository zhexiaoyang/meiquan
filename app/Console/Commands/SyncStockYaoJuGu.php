...
class SyncStockYaoJuGu extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync-stock-yaojugu';
    ...
    public function handle()
    {
        ...
        foreach ($shops_minkang as $name => $id) {
            ...
            $data = DB::connection('yaojugu')
                ->select("SELECT 商品编码 as id, 库存 as stock FROM [dbo].[v_busi_store] WHERE [美团门店ID] = N'{$id}' AND [商品条形码] <> '' AND [商品条形码] IS NOT NULL");
            if (!empty($data)) {
