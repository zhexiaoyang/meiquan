...
class SyncStockYongQinTangReset extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync-stock-yongqintang-reset {num}';
    ...
    public function handle()
    {
        ...
        for ($i = 1;$i <= 35; $i++) {
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $product) {
                    if (empty($product['barcode']) || empty($product['pid'])) {
                        continue;
                    }
                    $upc = $product['barcode'];
                    $pid = $product['pid'];
        ...
        //     $stock_data = $this->modifyStock($shop['yid'] , date("Y-m-d", strtotime('-62 day')));
        ...
        //             ];
        //         }
        //         $this->info("{$shop['name']}-第{$key}批总数：" . count($upc_data));
        //
        //         // 绑定编码
